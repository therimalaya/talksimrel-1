...
\rho_w^2 &= \Sigma_{ww}^{-1/2} \Sigma_{zw}^{t} \Sigma_{zz}^{-1} \Sigma_{zw} \Sigma_{ww}^{-1/2} \\
 &= \frac{\sigma_{ij}^t \Lambda ^{-1} \sigma_{ij'}}{\sqrt{\sigma_j^2\sigma_{j'}^2}} \forall j, j' = 1 \ldots m
\end{aligned}
$$
...
## Simulation Parameters {.columns-2 .plain}
...
## The Covariance Structure {.columns-3 .plain}
...
## Accessing Properties of Data {.columns-3 .plain}
...
## Application of simrel {.flexbox .vcenter .plain}

<h3>Research</h3>
<h3>Education</h3>

## Shiny Application and Installation {.flexbox .vcenter .plain}
...
## Acknoledgement {.columns-2 .plain}
